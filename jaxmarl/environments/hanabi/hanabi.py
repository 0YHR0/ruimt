...
class State:
    ...
    player_hands: chex.Array# cards currently hold by each agent
    info_tokens: chex.Array
    terminal: bool
    life_tokens: chex.Array
    card_knowledge: chex.Array#Tracks what each player knows about the cards in their own hand, based on the clues received for each card, using 25 bit to represent its rank and color combi
    colors_revealed: chex.Array#Tracks which colors have been revealed to each player through clues.
...
class HanabiGame(MultiAgentEnv):
    ...
    def __init__(self, num_agents=2, num_colors=5, num_ranks=5, hand_size=5, max_info_tokens=8, max_life_tokens=3,
        ...
        self.num_ranks = num_ranks # 5 nums in total
        self.hand_size = hand_size # can hold 5 cards in total
        self.max_info_tokens = max_info_tokens
        self.max_life_tokens = max_life_tokens
        self.num_cards_of_rank = num_cards_of_rank # 每个数字有几张
        self.deck_size = jnp.sum(num_cards_of_rank) * num_colors # 牌堆数量
        ...
        self.action_set = jnp.arange(self.num_moves) # 可以执行的操作，全局唯一
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[Dict, State]:
        ...
        shuffled_pairs = jax.random.permutation(_key, color_rank_pairs, axis=0)#shuffle the cards
    ...
    def get_obs(self, state: State) -> Dict:
        ...
        @partial(jax.vmap, in_axes=[0, None])
        # in_axes=[0, None] means that the first argument to the function will be vectorized along its first axis (axis 0), and the second argument will not be vectorized (hence None).
        #if you have a function f(x, y) and you decorate it with @partial(jax.vmap, in_axes=[0, None]),
        # f will now be able to take a vector (array) as its first argument x and apply the function to each element of this vector,
        # while the second argument y will be kept constant for all these operations.
    ...
    def step_agent(self, key: chex.PRNGKey, state: State, aidx: int, action: int,
        ...
        def _hint_fn(state, action):
            ...
            # 被提示的player的card knowledge被取出， 因为被提示了之后他对于自己的card knowledge改变了
            #得到没有被提示之前的card knowledge
            cur_knowledge = state.card_knowledge.at[hint_player].get()

            # get relevant player's hand
            #得到被提示的人手上有哪些怕牌，playerhands是一个agentnum个数的5x（5x5）的数组，最后的5x5是一个二维one hot coding
            cards = state.player_hands.at[hint_player].get()
            #card是一个5x5x5 的数组，axis=2是rank维度 axis=1是color维度 对着那一维度相加就是去除这一个维度的信息
            card_colors = jnp.sum(cards, axis=2)
            card_ranks = jnp.sum(cards, axis=1)

            # check which cards have hinted color/rank by the player with aidx index
            #是一个5x1的矩阵 如果为1哪几张牌被提示了，如果都为0则代表提示错误 不可能发生
            color_hint_matches = jnp.matmul(card_colors, hint_color)
            rank_hint_matches = jnp.matmul(card_ranks, hint_rank)
            #反提示 如果在color_hint_matches 中第 1 张牌为0，就是在提示它第一张牌不是hint color的颜色 5x25
            negative_color_hints = jnp.outer(1 - color_hint_matches, hint_color)
            #负面颜色提示（表示不匹配给定颜色提示的卡牌应排除的颜色信息）扩展到与当前玩家卡牌知识矩阵相同的形状，以便可以直接在卡牌知识更新中使用它。
            ...
            #基于给出的提示 更新被提示玩家的colorsrevealed 和ranks reveaedl
        ...
        1 1 1 0 0
        1 1 1 1 0
        1 1 1 1 1,下面这行代码就是把数组中所有值相加
        """
        fireworks_before = jnp.sum(state.fireworks, axis=(0, 1))

        # 如果是is_hint，执行_hint_fn,否则执行_discard_play_fn，这两个函数的参数都是（当前的state， 当前选择的action）
        state = lax.cond(is_hint[0], _hint_fn, _discard_play_fn, state, action)
