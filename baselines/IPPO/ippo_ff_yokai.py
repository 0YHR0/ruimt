    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                info['returned_episode'] = jax.tree_map(lambda x: x.reshape((config["NUM_ACTORS"])), info['returned_episode'])
            ...
            # traj_batch.info['shaped_reward'] = traj_batch.shaped_reward * train_state.shaped_reward_coeff
            # new_shaped_reward = traj_batch.shaped_reward * train_state.shaped_reward_coeff
            ...
            #     shaped_reward=new_shaped_reward
            # )
            ...
            advantages, targets = _calculate_gae(traj_batch, last_val, 1.0)
            # plot loss values
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        #todo log the total loss, value loss and actor loss
            ...
            value_loss,loss_actor,entropy = loss_info[1]
            train_state = update_state[0]
            metric = traj_batch.info
            rng = update_state[-1]

            def callback(metric,train_state,traj_batch,advantages, targets,total_loss,value_loss,loss_actor,entropy):
                wandb.log(
                    {
                        "total_rewards":  1*traj_batch.shaped_reward.sum(axis=0).mean(axis=-1) + traj_batch.reward.sum(axis=0).mean(axis=-1), #  [jnp.array([13, 27, 41, 55,69,83,97,111]), :].mean(),
                        "total_rewards_per_game_estimate":  (1*traj_batch.shaped_reward.sum(axis=0).mean(axis=-1) + traj_batch.reward.sum(axis=0).mean(axis=-1)) / (traj_batch.reward.shape[0]/14),
                        "env_step": metric["update_steps"]
                        * config["NUM_ENVS"]
                        * config["NUM_STEPS"],
                        "shaped_coefficient": 1,
                        "scaled_shaped_reward": 1*traj_batch.shaped_reward.sum(axis=0).mean(),
            ...
            # jax.debug.breakpoint()
            jax.experimental.io_callback(callback, None, metric, train_state, traj_batch,advantages, targets,total_loss,value_loss,loss_actor,entropy)
            # jax.debug.print('-------------\nmetrics mean return = {a}', a=metric["returned_episode_returns"][-1, :].mean())
            update_steps = update_steps + 1
            jax.debug.print("real_dsteps: {b}",
                             b=update_steps * config["NUM_ENVS"] * config["NUM_STEPS"])
            # new_coeff = 1 - update_steps/config["NUM_UPDATES"]
            # train_state = train_state.set_new_shaped_reward_coeff(
            #     new_coeff=new_coeff # Calcuclate just here
            # )
